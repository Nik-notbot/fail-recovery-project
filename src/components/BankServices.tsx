...
export function BankServices() {
  ...
  const logoFallbacks: Record<string, string> = {
    ...
    'revolut': 'https://play-lh.googleusercontent.com/9Agn2TZHIyOR4GGB4LcOU4VXwE2OcK47qwGY8XPZXz4u55ehNfpGuEBeNq_cbvH0z5A',
    'redotpay': 'https://play-lh.googleusercontent.com/kvP-BmU-swM_AqJSTgCFZEc44L2qgC32LWF2xctra2Px8FkSriGv9D7aqMp5FU2u2Q',
      ...
      <div className="container px-4 md:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10">
          {t("bankServices.banks").map((bank: any) => {
            ...
            return (
              <Card key={bank.id} className="overflow-hidden border-none shadow-lg transition-all duration-300 hover:shadow-xl flex flex-col h-full">
                <CardContent className="p-6 flex flex-col items-center h-full">
                  <div className="h-20 w-20 mb-6 flex items-center justify-center bg-white rounded-full overflow-hidden shadow-sm border border-gray-100">
                    <img 
                      ...
                      onError={(e) => {
                        (e.target as HTMLImageElement).src = logoFallbacks[logoKey] || '/placeholder.svg';
                      }}
                      alt={bank.name} 
                      className="max-h-14 max-w-14 object-contain rounded-full"
                      onLoad={() => console.log(`Успешно загружено: ${bank.name}`)}
                      onErrorCapture={() => console.log(`Ошибка загрузки: ${bank.name}`)}
                    />
                  </div>
                  <h3 className="text-xl font-bold mb-3">{bank.name}</h3>
                  <p className="text-center text-muted-foreground mb-auto">{bank.description}</p>
                  <div className="w-full mt-6">
                    <Button variant="outline" className="w-full border-gray-300 text-gray-700 hover:bg-gray-100" onClick={openTelegram}>
          ...
          <Card className="overflow-hidden border-none shadow-lg transition-all duration-300 hover:shadow-xl flex flex-col bg-gradient-to-br from-gray-50 to-gray-100 max-w-3xl w-full">
            <CardContent className="p-8 flex flex-col md:flex-row items-center gap-8">
              ...
              <div className="md:w-3/4 space-y-4 text-center md:text-left">
                <h3 className="text-2xl font-bold">{t("bankServices.customRequest.title")}</h3>
                <p className="text-muted-foreground">
                  {t("bankServices.customRequest.description")}
                </p>
                <Button className="bg-gray-900 hover:bg-gray-800 text-white px-8 rounded-full" onClick={openTelegram}>
                  <MessageSquare className="mr-2 h-4 w-4" /> {t("bankServices.customRequest.button")}
                </Button>
              </div>
            </CardContent>
          </Card>
